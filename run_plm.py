...
def main():
    ...
    if len(sys.argv) == 2 and (sys.argv[1].endswith(".yml") or sys.argv[1].endswith(".yaml")) :
        # If we pass only one argument to the script and it's the path to a yaml file,
        # let's parse it to get our arguments.
        with open(os.path.abspath(sys.argv[1])) as yaml_file_handle :
            args_dict = yaml.safe_load(yaml_file_handle)
    ...
    if model_args.tokenizer_name:
        # Cannot use AutoTokenizer with Rostlab/prot_xlnet
        #tokenizer = AutoTokenizer.from_pretrained(model_args.tokenizer_name, **tokenizer_kwargs)
        from transformers import XLNetTokenizer
        tokenizer = XLNetTokenizer.from_pretrained(model_args.tokenizer_name, do_lower_case=False)
    elif model_args.model_name_or_path:
